...
node_t *node_constructor(char *arg_name, char *arg_value, node_t *arg_left,
    ...
    if ((pthread_rwlock_init(&new_node->rwlock, NULL))) {
        free(new_node->name);
        free(new_node->value);
        free(new_node);
        return 0;
    ...
    if ((snprintf(new_node->name, MAXLEN, "%s", arg_name)) < 0) {
        free(new_node->value);
        free(new_node->name);
        pthread_rwlock_destroy(&new_node->rwlock);
        free(new_node);
        return 0;
...
void db_query(char *name, char *result, int len) {
    ...
    if ((err = lock(&head.rwlock, l_read))) {
        handle_error_en(err, "rwlock lock");
    }
    ...
    if (target == 0) {
        ...
        if ((err = pthread_rwlock_unlock(&target->rwlock))) {
            handle_error_en(err, "rwlock unlock");
        }
    }
...
int db_add(char *name, char *value) {
    ...
    if ((err = lock(&head.rwlock, l_write))) {
        handle_error_en(err, "rwlock lock");
    }

    if ((target = search(name, &head, &parent, l_write)) != 0) {
        if ((err = pthread_rwlock_unlock(&target->rwlock)) ||
            (err = pthread_rwlock_unlock(&parent->rwlock))) {
            handle_error_en(err, "rwlock unlock");
        }
        return (0);
    }
    ...
    if ((err = pthread_rwlock_unlock(&parent->rwlock))) {
        handle_error_en(err, "rwlock unlock");
    }
    return (1);
}
...
int db_remove(char *name) {
    ...
    if (dnode->rchild == 0) {
        ...
        // done with dnode
        if ((err = pthread_rwlock_unlock(&dnode->rwlock))) {
            handle_error_en(err, "rwlock unlock");
        }
        if ((err = pthread_rwlock_unlock(&parent->rwlock))) {
            handle_error_en(err, "rwlock unlock");
        ...
        while (next->lchild != 0) {
            ...
            if ((err = lock(&nextl->rwlock, l_write))) {
                handle_error_en(err, "rwlock lock");
            }
            pnext = &next->lchild;
            if ((err = pthread_rwlock_unlock(&next->rwlock))) {
                handle_error_en(err, "rwlock unlock");
            }
            next = nextl;
        }
...
void db_print_recurs(node_t *node, int lvl, FILE *out) {
    ...
    if ((err = lock(&node->rwlock, l_read))) {
        handle_error_en(err, "rwlock lock");
    }
    ...
    if ((err = pthread_rwlock_unlock(&node->rwlock))) {
        handle_error_en(err, "rwlock unlock");
    }
}
